...
namespace Domain
{
    ...
    {
        private readonly CardBasedStrategy _rankBasedStrategyCard;
        private readonly ValueBasedStrategy _valueBasedStrategyCard;

        public FourDeckBasicStrategy()
        {
        ...
        public static FourDeckBasicStrategy Instance
        {
            get
            {
                return Singleton<FourDeckBasicStrategy>.GetInstance(() => new FourDeckBasicStrategy());
            }
        }
        ...
        public DecisionType GetDecision(Hand playerHand, Card dealerUpCard)
        {
            ...
            try
            {
                return _rankBasedStrategyCard.Get(playerHand[0].Rank, playerHand[1].Rank, playerHand.Value,
                    dealerUpCard.Rank);
            }
            catch (Exception)
            {
            ...
            throw new DecisionNotFoundException(
                String.Format("Decision Not Found for player cards {0} and {1} with value {2}, dealerUpCard {3}",
                    playerHand[0], playerHand[1], playerHand.Value, dealerUpCard));
        }

        private class CardBasedStrategy
        {
            private readonly Dictionary<CardRank, Dictionary<CardRank, Dictionary<int, Dictionary<CardRank, DecisionType>>>> _strategy;
        ...
        {
            private readonly Dictionary<int, Dictionary<CardRank, DecisionType>> _strategy;
