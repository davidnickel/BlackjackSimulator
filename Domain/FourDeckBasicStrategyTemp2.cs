...
namespace Domain.Temp2
{
    public class FourDeckBasicStrategyTemp2 : IBasicStrategy
    {
        private CardBasedStrategy _rankBasedStrategyCard;
        private ValueBasedStrategy _valueBasedStrategyCard;

        public FourDeckBasicStrategyTemp2()
        {
        ...
        {
            private Dictionary<string, DecisionType> _strategy;

            public CardBasedStrategy()
            {
                _strategy = new Dictionary<string, DecisionType>();
            }

            public DecisionType Get(CardRank firstCard, CardRank secondCard, int handValue, CardRank dealerCard)
            {
                string key = String.Format("{0}|{1}|{2}|{3}", firstCard, secondCard, handValue, dealerCard);
        ...
        private class ValueBasedStrategy
        {
            ...
            {
                string key = String.Format("{0}|{1}", handValue, dealerCard);
                _strategy.Add(key, decision);
            }
